    ...
    <div className="flex items-center p-2 rounded-lg hover:bg-white/10 transition-colors">
      <div
        className={`h-8 w-8 rounded-full flex items-center justify-center mr-2 ${
          transaction.transaction_type === "CREDIT" ? "bg-green-100/20" : "bg-amber-100/20"
        }`}
      >
        <Icon className={`h-5 w-5 ${transaction.transaction_type === "CREDIT" ? "text-green-300" : "text-amber-300"}`} />
      </div>

      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-center">
          <div>
            <p className="font-medium text-sm text-white/90 truncate">{transaction.description}</p>
            <p className="text-[12px] text-white/60">{transaction.created_at}</p>
          </div>
          <div className="text-right">
            <p
              className={`font-semibold text-sm ${transaction.transaction_type === "CREDIT" ? "text-green-300" : "text-amber-300"}`}
            >
              {transaction.transaction_type === "CREDIT" ? "+" : "-"} ₦{transaction.amount.toLocaleString("en-NG")}
            </p>
          </div>
        </div>
      </div>
